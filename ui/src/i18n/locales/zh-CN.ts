const messages = {
  ...
  jobs: {
    ...
    deleteConfirm: '删除该任务？这也会删除其运行历史。',
    deleteTitle: '删除任务',
    deleteHelp: '归档会停止调度并保留历史；永久删除会删除任务及其运行历史。',
    deletePermanentlyHelp: '永久删除会删除任务及其运行历史。',
    archiveCascadeLabel: '归档时级联删除快照',
    archiveCascadeHelp: '对该任务现存快照创建删除任务（后台执行）；已固定的快照会被排除。',
    archived: '已归档',
    showArchived: '显示已归档',
    fields: {
      name: '名称',
    ...
    retention: {
      title: '保留策略',
      help: '自动管理快照保留：保留最近 N 份 / 最近 N 天；已固定（Pinned）的快照不会被保留策略删除。',
  ...
  settings: {
    ...
    hubRuntimeConfig: {
      ...
      fields: {
        ...
        backupRetentionEnabled: '启用备份保留默认值',
        backupRetentionEnabledHelp: '启用后，新建任务将默认带上此保留策略（可在任务编辑中覆盖）。',
        ...
        backupRetentionTips: '提示：启用后需至少设置“保留最近 N 份”或“保留最近 N 天”之一；固定（Pinned）的备份不会被保留策略删除。',
        logFilter: '日志过滤器',
        logFile: '日志文件',
        logRotation: '日志轮转',
        logKeepFiles: '保留文件数',
