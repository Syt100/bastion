const messages = {
  ...
  agents: {
    ...
    tokenModal: {
      ...
      create: 'Create',
      help: 'Use this token when enrolling an Agent',
      token: 'Token',
    ...
    bulkSyncModal: {
      ...
      help: 'Queue delivery of the latest config snapshot to multiple Agents',
      target: 'Target',
      targetSelected: 'Selected agents ({count})',
      targetLabelFilter: 'Label filter',
    },
  ...
  jobs: {
    ...
    fields: {
      ...
      artifactFormatRawTreeHelp: 'Stores files under data/<path> with best-effort metadata (encryption not supported).',
      ...
      vaultwardenDataDirHelp: 'Bastion snapshots db.sqlite3 under this directory without downtime',
      ...
      notifyDisabledSelected: 'Selected destinations include disabled ones: {names} (will send when re-enabled)',
    },
    nodes: {
      hub: 'Hub (local)',
    },
    ...
    retention: {
      title: 'Retention',
      help: 'Automatically prune snapshots: keep the last N, and/or keep snapshots from the last N days. Pinned snapshots are never deleted automatically.',
  ...
  errors: {
    ...
    webdavPrefixRootNotAllowed: 'Please select a subdirectory (root is not allowed)',
