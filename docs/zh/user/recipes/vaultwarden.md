...
2. 将 **Vaultwarden 数据目录** 设置为挂载目录对应的宿主机路径：
   - 示例：`/opt/vaultwarden/data`
3. 选择一个备份目标：
   - **本地目录**（最简单）：`/opt/bastion-backups`
   - **WebDAV**：先在 **设置 → 存储** 中配置 WebDAV 凭据
4. （可选）启用备份加密（age）
...
使用 UI 中的 **校验** 做端到端完整性检查：

- 下载快照
- 恢复到临时目录
- 做文件哈希校验
- 对 `db.sqlite3` 执行 `PRAGMA integrity_check`
