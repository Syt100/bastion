...
- **Agent**（可选）：连接到 Hub，在另一台机器上执行作业。

## 典型使用流程

1. [启动 Hub 并完成首次初始化](/zh/user/getting-started)。
2. （可选）[接入 Agents](/zh/user/agents) 来做多节点备份。
3. 创建 [Jobs（作业）](/zh/user/jobs) 并运行。
4. 查看 [Runs（运行记录）](/zh/user/runs)，并通过 [恢复与校验](/zh/user/restore-verify) 做恢复演练与一致性检查。
...
- [批量操作](/zh/user/bulk-operations)（labels、批量同步配置、分发 WebDAV 凭据、批量部署作业）
- [运维](/zh/user/operations/reverse-proxy)（反向代理 / 运行时配置 / 日志 / 维护 / 数据目录）
