...
use crate::smtp::{SmtpSecretPayload, SmtpTlsMode};
...
#[derive(Debug, Deserialize)]
struct UpsertSmtpSecretRequest {
    host: String,
    port: u16,
    username: String,
    password: String,
...
pub fn router(state: AppState) -> Router {
    Router::new()
        ...
        .route("/api/secrets/smtp", get(list_smtp_secrets))
        .route(
            "/api/secrets/smtp/{name}",
            get(get_smtp_secret)
                .put(upsert_smtp_secret)
                .delete(delete_smtp_secret),
        )
