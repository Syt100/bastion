...
#[tokio::test]
async fn bulk_operations_create_missing_payload_has_structured_reason_and_field() {
    let temp = TempDir::new().expect("tempdir");
    let pool = db::init(temp.path()).await.expect("db init");
...
#[tokio::test]
async fn bulk_operations_create_missing_selector_has_structured_reason_and_field() {
    let temp = TempDir::new().expect("tempdir");
    let pool = db::init(temp.path()).await.expect("db init");
...
#[tokio::test]
async fn bulk_operations_create_invalid_kind_has_structured_reason_and_field() {
    let temp = TempDir::new().expect("tempdir");
    let pool = db::init(temp.path()).await.expect("db init");
    ...
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    let body: serde_json::Value = resp.json().await.expect("json");
    assert_eq!(body["error"].as_str().unwrap_or_default(), "invalid_kind");
    assert_eq!(
        body["details"]["reason"].as_str().unwrap_or_default(),
        "unsupported_value"
    );
