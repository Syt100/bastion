...
- Fixed run event detail readability by adding a summary → key diagnostics → context evidence layout, exposing run/request identifiers in the header, and using progressive disclosure for long values, `error_chain`, and raw JSON sections.
