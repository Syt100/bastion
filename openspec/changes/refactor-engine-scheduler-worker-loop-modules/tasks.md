...
- [ ] 2.1 Identify responsibilities and module boundaries (args, worker loop orchestration, dispatch/execute submodules)
- [ ] 2.2 Extract worker loop implementation into `crates/bastion-engine/src/scheduler/worker/loop.rs`
- [ ] 2.3 Keep `WorkerLoopArgs` and `run_worker_loop` entrypoints stable in `worker/mod.rs`
