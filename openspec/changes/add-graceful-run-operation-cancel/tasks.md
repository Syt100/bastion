...
- [ ] 2.1 Add DB migration and model updates for `canceled` status + cancel-request metadata (runs/operations)
- [ ] 2.2 Add repository helpers for idempotent cancel requests and CAS finalization guards
- [ ] 2.3 Add authenticated cancel APIs: `POST /api/runs/{id}/cancel` and `POST /api/operations/{id}/cancel`
- [ ] 2.4 Add scheduler/worker cancellation registry and immediate wakeup signaling for active local tasks
- [ ] 2.5 Ensure queued tasks canceled before dispatch never start execution
...
- [ ] 5.1 Add backend regression tests for queued-cancel, running-cancel, idempotency, and late-result race safety
