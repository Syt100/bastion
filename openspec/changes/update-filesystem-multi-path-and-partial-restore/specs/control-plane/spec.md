...
The system SHALL provide an API to browse archived paths for a completed run to support restore selection.

#### Scenario: Browse run entries by prefix
- **WHEN** the user requests entries for a run under a given prefix
- **THEN** the API returns matching archive paths with pagination
